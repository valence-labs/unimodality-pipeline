...
def tvn_on_controls(
    ...
    if batch_col is not None:
        ...
        for batch in batches:
            ...
            source_cov_half_inv = (linalg.fractional_matrix_power(source_cov, -0.5)).astype(float)
            embeddings[batch_ind] = np.matmul(embeddings[batch_ind], source_cov_half_inv)
    return embeddings



def center_scale_process(adata, obsm_key, batch_co):
    center_scaled_embeddings = tvn_on_controls(
                        embeddings=adata.obsm[obsm_key],
                        metadata=adata.obs,
                        pert_col='is_control',
                        control_key=True,
                        batch_col=batch_co)
    return center_scaled_embeddings
