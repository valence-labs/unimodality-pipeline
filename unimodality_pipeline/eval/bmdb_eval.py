...
BENCHMARK_DATA_DIR = "./resources/bmdb_database"
BENCHMARK_SOURCES = ["CORUM", "HuMAP", "Reactome", "SIGNOR", "StringDB", "TxDB", "PhDB"]
RECALL_PERC_THRS = [(0.05, 0.95)]
RANDOM_SEED = 42
N_NULL_SAMPLES = 5000
MIN_REQ_ENT_CNT = 20
...
def compute_bmdb_scores(metadata, emb):
    map_data = aggregate(emb, metadata, pert_col='gene_name', keys_to_remove=['non-targeting'])
    metrics = known_relationship_benchmark(map_data, recall_thr_pairs=RECALL_PERC_THRS, 
                            pert_col='gene_name', simulate_perfect_recall=False)
    result_col = f"recall_{RECALL_PERC_THRS[0][0]}_{RECALL_PERC_THRS[0][1]}"
