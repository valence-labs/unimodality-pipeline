...
from ..tools.sigclip_loss import SigClipLoss  # Assuming you save the SigClipLoss in sigclip_loss.py
...
class SigClipModule(LightningModule):
    def __init__(
        self, 
        ...
        # SigCLIP loss
        self.loss = SigClipLoss(
            temperature=self.hparams.temperature,
            normalize=self.hparams.normalize,
            bias=self.hparams.bias,
            learnable_bias=self.hparams.learnable_bias,
    ...
    def training_step(self, batch, batch_idx):
        ...
        z_tx_original = x_tx  # Assuming x_tx is already an embedding
        ...
        if self.current_epoch == 0:
            self.knn_classifier_input.update(train_features=z_tx_original.detach(), train_targets=labels)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.current_epoch == 0:
            self.knn_classifier_input.update(test_features=z_tx_original.detach(), test_targets=labels)
    ...
    def configure_optimizers(self):
        ...
        # Add the bias parameter if it's learnable
        if self.loss.bias.requires_grad:
            parameters.append({"params": [self.loss.bias], "lr": self.hparams.bias_lr})
